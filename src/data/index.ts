...
export const mockChat = [
  {
    id: "1",
    role: "user",
    content: "почему сортировка не работает",
  ...
  },
  {
    id: "5",
    role: "user",
    content: "почему сортировка не работает groupedChats = Object.entries( groupBy(chats, \"createdAt\", (time: string) => { return new Date(time).toISOString().split(\"T\")[0]; }) ).sort((a, b) => new Date(a[0]).getTime() - new Date(b[0]).getTime()); ``` Если хочешь обратно получить объект — оберни в `Object.fromEntries(...)`.",
    createdAt: "2025-07-21T21:30:00.000Z",
  },
  {
    id: "6",
