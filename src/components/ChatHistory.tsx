            ...
            <ul className="flex flex-col gap-2">
              {chats.map((chat: TChat) => (
                <li key={chat.id}>
                  <Link
                    href={`/chat/${chat.id}`}
                    className="w-full rounded-xl bg-neutral-100 px-3 py-1 text-sm backdrop-blur-sm backdrop-filter transition-colors focus:outline-2 focus:outline-offset-2 focus:outline-blue-500"
                  >
                    {chat.name}
                  </Link>
                </li>
